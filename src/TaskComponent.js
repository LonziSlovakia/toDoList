...
function Task({ task, color, funcs: {softDelete, hardDelete, complete} }) {
    ...
    return (
        <li className={`py-1 px-2 border-b-2 border-t-2 border-${color}-700 text-${color}-900 ${softDeletedClasses} flex items-center`}>
            ...
            <div><input type="checkbox" className="form-checkbox ml-6 mr-1 mb-1 p-2"  onClick={handleCompleteTask} checked={task.completed} /></div>
